    ...
    <div class='channel_header'>
      <div class='channel_header_user_info'>
        ...
        ?>
           ...
           <div class='channel_header_navi'>
              ...
              echo "<div class='channel_navi_btn no_overflow channel_navi_btn_activ'>        <a href='".$_dhp."user/".$channel_name."/videos'>".$l->video_navi_title."</a></div>";
        ...
        if($isUserLoggedIn === 1){
            ...
            if($friend_row['friend_id'] != ""){
              $vid_results = db::$link->query("SELECT COUNT(vuid) FROM video_db WHERE uuid = '$channel_uuid' AND status = 'uploaded' AND (privacy ='public' OR privacy = 'friend')");
              ...
              $vid_results = db::$link->query("SELECT COUNT(vuid) FROM video_db WHERE uuid = '$channel_uuid' AND status = 'uploaded' AND privacy ='public'");
            }
        }else{
          $vid_results = db::$link->query("SELECT COUNT(vuid) FROM video_db WHERE uuid = '$channel_uuid' AND status = 'uploaded' AND privacy ='public'");
        }
          ...
          echo"<div class='allvideoTitle no_overflow'>";
            echo"<div class='allvideoCount' class='left'>".$l->allvideos_from_title." ".$channel_name." (".$allvideoCount.")</div>";
            ?>
            <selecter class='sort_selector'>

            <form method="POST" >
                <selecter_text><?php echo $l->sortiert_nach.": ";?><select name="sort_art" id="sort" class="land" onChange="gotosite(''+this.options[this.selectedIndex].value,'','0');"></selecter_text>
                ...
                <?php
                  ...
                  if(isset($_GET['sort'])){$sort = $_GET['sort'];}else{$sort = 2;}
                            ...
                            $sort3_l = $video."&sort=3";
                            $sort2_l = $video."&sort=2";
                            $sort1_l = $video."&sort=1";
                            $sort0_l = $video."&sort=0";

                        if($sort === '3'){
                            echo	"<option value='".$sort3_l."'>".$l->sort_title4."</option>";
              ...
              $(document).ready(function() {
                ...
                $(".load_more").click(function (e) {
                  ...
                  $('.animation_image').show();
                    ...
                    {
                        $.post('<?php echo $_dhp; ?>ajax/channel_videos', {'sort':'<?php echo $sort; ?>', 'u':'<?php echo $channel_uuid; ?>', 'page': track_click}, function(data) {
                          $(".load_more").show();
                          $("#results").append(data);
      ...
      <div class='row'>
        <div class='col-xs-12 col-spl'>
          <div id="results" class='channel_video_results'>
            <?php require_once ($_hp."ajax/channel_videos.php"); ?>
          </div>
            <div align="center">
